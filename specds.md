...
### Задача продажи билетов (прогнозирование поведения). Графовые модели

**Постановка задачи:**

Система продажи билетов на транспорт должна оптимизировать размещение пассажиров и ценообразование на основе прогнозирования их поведения.

**Ключевые вопросы для решения:**

1. **Какое место выберет следующий пассажир?**
   - Прогнозирование предпочтений по расположению мест
   - Анализ паттернов выбора (окно/проход, верх/низ)

2. **Какое место выберет данный пассажир?**
   - Персонализированное предсказание на основе истории
   - Учет индивидуальных предпочтений конкретного пользователя

3. **Исходя из количества и типа оставшихся мест, надо ли менять цены (динамическое ценообразование)?**
   - Оптимизация выручки на основе остатков
   - Балансировка между заполняемостью и прибылью
   - Учет времени до отправления

4. **Надо ли добавлять вагоны в состав?**
   - Прогноз спроса на маршрут
   - Оптимизация загрузки составов
   - Минимизация затрат на перевозку

**Графовая модель:**

Задача решается построением стохастического графа, где:
- **Узлы** — возможные состояния (свободные места, текущие цены)
- **Ребра** — вероятности переходов (выбор конкретного места пассажиром)
- **Веса** — вероятности, основанные на исторических данных и характеристиках пассажиров

### Анализ по дереву решений (Decision Tree)

**Определение:**

**Дерево** — однонаправленный граф с непересекающимися ветвями.

Дерево решений — это иерархическая структура, используемая для принятия решений и классификации данных. Каждый внутренний узел представляет собой условие (правило разделения), каждая ветвь — результат проверки условия, а листовые узлы — конечные решения или классы.

**Основные свойства дерева:**
- Имеет корневой узел (начальную точку)
- Ветви не пересекаются (нет циклов)
- Каждый узел (кроме корня) имеет ровно одного родителя
- Направление — от корня к листьям

**Применение деревьев решений:**
- Классификация и регрессия в машинном обучении
- Системы принятия решений
- Анализ сценариев и стратегическое планирование
- Диагностические системы

### Алгоритм поиска по дереву решений

**Основные шаги алгоритма:**

1. **Включить все имеющиеся данные в корневой узел**
   - Начальный набор данных помещается в корень дерева

2. **Выбрать атрибут, оценка по которому дает наилучший результат**
   - Используются метрики качества разделения (энтропия, Gini-индекс)
   - Выбирается признак, который минимизирует количество/долю ошибок

3. **Добавить на следующем уровне дерева узлы**
   - Создаются дочерние узлы, соответствующие остальным атрибутам/параметрам оценки
   - Данные распределяются по ветвям согласно выбранному условию

4. **Провести оценку по атрибутам**
   - Для каждого подмножества данных повторяется оценка оставшихся признаков

5. **Повторять предыдущие шаги, пока не будет достигнут критерий остановки**

**Критерии остановки:**
- Достигнуто предельно установленное количество узлов (глубина дерева)
- Достигнут приемлемый уровень точности
- В узле осталось минимальное количество объектов
- Все объекты в узле принадлежат одному классу

**Пример: Классификация клиентов для предложения кредита**

Задача: определить, стоит ли предлагать кредит клиенту банка.
...
```mermaid
graph TD
    Root["Корень: Все клиенты<br/>100 человек"]
    
    Root -->|Кредитная история| CH_Good["Хорошая<br/>60 чел"]
    Root -->|Кредитная история| CH_Bad["Плохая<br/>40 чел"]
    
    CH_Good -->|Доход| Inc_High["Высокий<br/>40 чел<br/>✅ Одобрить"]
    CH_Good -->|Доход| Inc_Low["Низкий/Средний<br/>20 чел"]
    
    CH_Bad -->|Наличие работы| Job_Yes["Есть работа<br/>25 чел"]
    CH_Bad -->|Наличие работы| Job_No["Нет работы<br/>15 чел<br/>❌ Отказать"]
    
    Inc_Low -->|Возраст| Age_Young["< 30<br/>5 чел<br/>❌ Отказать"]
    Inc_Low -->|Возраст| Age_Mid["30-50<br/>15 чел<br/>✅ Одобрить"]
    
    Job_Yes -->|Доход| Inc2_High["Средний/Высокий<br/>20 чел<br/>✅ Одобрить"]
    Job_Yes -->|Доход| Inc2_Low["Низкий<br/>5 чел<br/>❌ Отказать"]
```

**Объяснение процесса:**

1. **Шаг 1**: Все 100 клиентов помещаются в корень
2. **Шаг 2**: Анализ показал, что "Кредитная история" дает лучшее разделение (минимальная ошибка)
3. **Шаг 3**: Создаются две ветви: хорошая история (60 чел) и плохая (40 чел)
4. **Шаг 4**: Для группы с хорошей историей лучший признак — "Доход", для плохой — "Наличие работы"
5. **Шаг 5**: Процесс продолжается, пока не достигнем критерия остановки

**Результат:**
- Клиенты в листовых узлах получают решение: ✅ одобрить или ❌ отказать
...
### Повышение производительности работы нейросетей (загрузка весовых коэффициентов)

Для эффективной работы с нейронными сетями и графовыми алгоритмами в задачах машинного обучения используются специализированные фреймворки:

#### 1. PyTorch

**PyTorch** — фреймворк глубокого машинного обучения на основе сверточных нейросетей.
...
- Оптимизированная загрузка и сохранение весовых коэффициентов
...
**Ссылка:** [pytorch.org](https://pytorch.org)

#### 2. PyBrain

**PyBrain** (Python-Based Reinforcement Learning, Artificial Intelligence and Neural Network Library) — Python библиотека для изучения и реализации большого количества разнообразных алгоритмов глубинного обучения.

**Основные возможности:**
- Модульная архитектура для построения нейросетей
- Реализация различных типов нейронных сетей
- Алгоритмы обучения с подкреплением
- Эволюционные алгоритмы
- Инструменты для оптимизации

**Типы поддерживаемых сетей:**
...
- Прототипирование нейросетевых моделей
- Образовательные проекты
- Эксперименты с различными архитектурами

**Ссылка:** [pybrain.org](http://pybrain.org)

### Структура PyBrain

PyBrain имеет модульную архитектуру, которая обеспечивает гибкий процесс разработки и обучения нейронных сетей.

**Основные компоненты:**

```mermaid
graph TD
    Raw[Сырые данные] --> Process[Обработка]
    
    Process --> Train[PyBrain Dataset<br/>Тренировка]
    Process --> Test[PyBrain Dataset<br/>Тестирование]
    
    subgraph PyBrain["PyBrain Framework"]
        Train --> Trainer[Учитель<br/>BP, SVM, Evolution]
        Trainer --> Model[Модель<br/>SVN, FFN, LSTM]
        Trainer --> Classify[Классификация]
        
        Test --> Valid[Валидация]
        Classify --> Valid
        Model --> Trainer
    end
    
    Valid --> Vis[Визуализация]
    Valid --> Results[Результаты]
```

**Описание компонентов:**

1. **Сырые данные** — исходный набор данных для обучения и тестирования

2. **Обработка** — предобработка данных (нормализация, очистка, feature engineering)

3. **PyBrain Dataset** — специальная структура данных PyBrain:
   - **Тренировочный набор** — данные для обучения модели
   - **Тестовый набор** — данные для проверки качества

4. **Модель** — типы нейронных сетей:
   - **SVN** (Support Vector Network) — метод опорных векторов
   - **FFN** (Feed-Forward Network) — прямая нейронная сеть
   - **LSTM** (Long Short-Term Memory) — рекуррентная сеть с долгой краткосрочной памятью

5. **Учитель (Trainer)** — алгоритмы обучения:
   - **BP** (Backpropagation) — метод обратного распространения ошибки
   - **SVM** (Support Vector Machine) — метод опорных векторов
   - **Evolution** — эволюционные алгоритмы

6. **Классификация** — применение обученной модели к данным

7. **Валидация** — проверка качества модели на тестовом наборе

8. **Визуализация** — графическое представление результатов и метрик

9. **Результаты** — финальные метрики качества модели (accuracy, precision, recall и т.д.)

## [Модуль 7] Нечеткая логика
...
- **Неизвестность** — полное отсутствие информации о каких-либо аспектах системы
- **Неполнота (недостаточность, неадекватность)** — частичное отсутствие необходимой информации
- **Недостоверность** — сомнительная достоверность имеющейся информации

### Типы недостоверности

Недостоверность бывает:

1. **Физической** — источником ее является внешняя среда (VUCA мир)
   - VUCA: Volatility (изменчивость), Uncertainty (неопределенность), Complexity (сложность), Ambiguity (неоднозначность)
   - Связана с непредсказуемостью и изменчивостью окружающей среды

2. **Лингвистической** — возникает в результате словесного обобщения и обусловливается необходимостью описания бесконечного числа ситуаций ограниченным числом слов в ограниченное время
   - Связана с ограниченностью естественного языка
   - Проявляется при попытке формализации нечетких понятий
   - Требует применения теории нечетких множеств для работы с такой неопределенностью

## «Специалист»: формы обучения/характеристики слушателей

Различные формы обучения имеют разную эффективность в зависимости от потребностей и характеристик слушателей.

### Сравнение форм обучения

| Форма обучения/Требования и характеристики | Очное | Вебинар | Открытое | Очно-заочное | Микролернинг |
|---------------------------------------------|-------|---------|----------|--------------|--------------|
| Обучиться быстро | *** | *** | ** | - | ***** |
| Дешево | * | ** | * | *** | *** |
| Индивидуальный темп | * | * | *** | *** | *** |
| «Разгильдяи» (хочу, чтобы контролировали) | *** | * | ** | * | - |
| Интроверты (Хочу, чтобы меня не трогали) | * | ** | *** | ** | *** |
| Экстроверты (Хочу, чтобы меня активно вовлекали) | *** | ** | - | * | - |
| Далеко добираться | | *** | | | *** |

### Описание форм обучения

1. **Очное обучение** — традиционный формат с личным присутствием
   - Отлично подходит для быстрого обучения
   - Идеально для тех, кто нуждается в контроле и дисциплине
   - Оптимально для экстровертов, желающих активного взаимодействия

2. **Вебинар** — онлайн-обучение в режиме реального времени
   - Позволяет быстро освоить материал
   - Доступно по цене
   - Решает проблему удаленного доступа

3. **Открытое обучение** — самостоятельное изучение материалов
   - Минимальная стоимость
   - Возможность учиться в собственном темпе
   - Подходит для интровертов и самоорганизованных студентов

4. **Очно-заочное** — смешанный формат обучения
   - Баланс между стоимостью и эффективностью
   - Гибкий темп обучения
   - Подходит для различных типов личности

5. **Микролернинг** — обучение короткими модулями
   - Максимальная скорость освоения материала
   - Доступная стоимость
   - Идеальная гибкость темпа обучения
   - Подходит для самостоятельного изучения

## [Модуль 8] Профессиональная ориентация по специальностям Data Science

### Основные роли

В сфере Data Science и аналитики данных существует множество специализированных ролей, каждая из которых выполняет важные функции в процессе работы с данными:

1. **Владелец продукта от заказчика** (Product Owner)
   - Представляет интересы бизнеса и заказчика
   - Определяет требования к продукту
   - Расставляет приоритеты в разработке
   - Принимает ключевые решения по функциональности

2. **Менеджер продукта (разработка)** (Product Manager)
   - Координирует процесс разработки
   - Управляет backlog задач
   - Обеспечивает взаимодействие между командами
   - Контролирует сроки и качество реализации

3. **Бизнес-аналитик** (Business Analyst)
   - Анализирует бизнес-процессы
   - Формулирует требования к системам
   - Создает спецификации и документацию
   - Обеспечивает связь между бизнесом и IT

4. **Аналитик данных** (Data Analyst)
   - Собирает и анализирует данные
   - Создает отчеты и визуализации
   - Выявляет тренды и закономерности
   - Поддерживает принятие решений на основе данных

5. **Ученый по данным** (Data Scientist)
   - Разрабатывает и применяет модели машинного обучения
   - Проводит глубокий статистический анализ
   - Решает сложные аналитические задачи
   - Создает предиктивные модели

6. **Разработчики в области больших данных** (Big Data Developers)
   - Разрабатывают решения для обработки больших объемов данных
   - Работают с распределенными системами (Hadoop, Spark)
   - Создают ETL-процессы
   - Оптимизируют производительность обработки данных

7. **Инженеры по системам хранения данных** (Data Storage Engineers)
   - Проектируют и поддерживают хранилища данных
   - Разрабатывают архитектуру баз данных
   - Обеспечивают надежность и доступность данных
   - Оптимизируют структуры хранения

8. **Инженеры по системам инфраструктуры** (Infrastructure Engineers)
   - Создают и поддерживают инфраструктуру для работы с данными
   - Настраивают серверы и сети
   - Обеспечивают масштабируемость систем
   - Автоматизируют процессы развертывания

9. **ИБ специалист по бигдата** (Big Data Security Specialist)
   - Обеспечивает безопасность данных
   - Разрабатывает политики доступа к данным
   - Проводит аудит безопасности
   - Защищает от утечек и несанкционированного доступа

10. **Промпт специалист** (Prompt Engineer)
    - Разрабатывает эффективные промпты для AI-моделей
    - Оптимизирует взаимодействие с языковыми моделями
    - Тестирует и улучшает качество ответов AI
    - Создает шаблоны и guidelines для работы с AI

11. **Специалист в области дискретной математики** (Discrete Mathematics Specialist)
    - Применяет методы дискретной математики в анализе данных
    - Разрабатывает алгоритмы оптимизации
    - Работает с графами и комбинаторными задачами
    - Обеспечивает математическое обоснование решений

### Навыки и компетенции, общие для всех ролей

Независимо от конкретной специализации, все профессионалы в области Data Science должны обладать определенным набором базовых навыков и компетенций:

#### 1. Понимание предметной области

- Глубокое знание бизнес-процессов и специфики отрасли
- Способность переводить бизнес-задачи в технические требования
- Понимание контекста применения данных и моделей
- Знание отраслевых стандартов и регуляций
- Умение выявлять ключевые метрики и KPI бизнеса

#### 2. Базовое понимание возможностей больших данных и машинного обучения

- Представление о технологиях обработки больших объемов данных
- Знание основных алгоритмов машинного обучения и их применимости
- Понимание ограничений и возможностей ML-моделей
- Осведомленность о современных инструментах и платформах
- Способность оценить целесообразность применения ML для конкретной задачи
- Понимание жизненного цикла ML-проектов

#### 3. Владение единым терминологическим аппаратом

- Использование согласованной терминологии в команде
- Точное понимание профессиональных терминов
- Способность объяснять технические концепции простым языком
- Знание отраслевых аббревиатур и сокращений
- Умение документировать решения с использованием стандартной терминологии
- Понимание различий в терминологии между смежными областями

#### 4. КОММУНИКАТИВНЫЕ НАВЫКИ!

**Критически важная компетенция для всех ролей в Data Science:**

- **Межкомандное взаимодействие**
  - Эффективная коммуникация с разными специалистами (разработчики, аналитики, менеджеры)
  - Способность работать в кроссфункциональных командах
  - Умение слушать и учитывать мнения коллег

- **Презентация результатов**
  - Четкое изложение технических решений нетехнической аудитории
  - Визуализация данных и результатов анализа
  - Подготовка понятных отчетов и презентаций
...
- **Документирование**
  ...
  - Ведение базы знаний
  - Описание процессов и методологий

- **Обратная связь**
  - Конструктивная критика и восприятие критики
  - Участие в code review и peer review
  - Mentoring и knowledge sharing

### Детальное описание ролей

#### Владелец продукта от заказчика (Product Owner)

**Область ответственности:**
- Постановка бизнес-целей и видения продукта
- Обеспечение ресурсами (бюджет, данные, эксперты)
- Постановка ограничений (сроки, бюджет, требования безопасности)

**Навыки:**
- Экспертиза в предметной области
- Управленческие и переговорные навыки
- Способность мотивировать команду
- Разработка метрик и управление по KPI

#### Менеджер продукта (Product Manager)

**Область ответственности:**
- Согласование содержания и управление backlog
- Достижение целей продукта (спринты, релизы, качество)
- Работа с командой по Agile/Scrum фреймворкам

**Навыки:**
- Понимание бизнеса и разработки
- Agile/Scrum, риск-менеджмент
- Переговорные навыки и мотивация команды
- Аналитические навыки и работа с метриками
- Базовые знания ML и MLOps

#### Аналитик данных (Data Analyst)

**Область ответственности:**
- Бизнес-анализ и поиск инсайтов
- Предоставление данных (сбор, очистка, дашборды, визуализация)
- Консультации команды по анализу данных

**Навыки:**
- Бизнес-анализ: аналитическое мышление, работа с требованиями, моделирование процессов
- SQL, инструменты визуализации (Tableau, Power BI, Python)
- Статистический анализ, A/B тестирование
- Базовое понимание ML, метрик качества, препроцессинга данных

#### «Ученый» по данным/Алгоритмист (Data Scientist)

**Область ответственности:**
- Взаимодействие с аналитиками, менеджерами, инженерами и программистами
- Разработка и адаптация математических моделей ML
- Консультации команды по ML и статистике
- Построение визуализаций и дашбордов моделей
- Обучение программистов математическим методам

**Навыки:**
- **ML методы:** supervised/unsupervised learning, deep learning, NLP, computer vision
- **Оценка качества данных** и препроцессинг
- **Нестандартное мышление** для feature engineering
- **Выявление трендов** через EDA
- **Инструменты:** Python (pandas, scikit-learn), ML frameworks (TensorFlow, PyTorch), Big Data (Spark), MLOps
- **Баланс:** "Больше программист, чем статистик и больше статистик, чем программист" (Excel → Matlab → C++)

#### Разработчики (инженеры) в области больших данных (Big Data Developers)

**Область ответственности:**
- Системный анализ
- Очистка данных
- Разработка и оптимизация прикладных решений (продуктов), в том числе встроенных
- Предоставление данных
- Взаимодействие с аналитиками, учеными по данным и дискретными математиками

**Навыки:**
- Понимание методов ИИ и машинного обучения
- Знание программных библиотек
- Умение выбирать и дорабатывать существующие библиотеки
- Владение фреймворками и языками программирования
- Навыки оптимизации и рефакторинга кода
- Умение интерпретировать полученные результаты

#### Инженеры по системам хранения данных (Data Storage Engineers)

**Область ответственности:**
- Архитектура данных
- Хранение и предоставление данных
- Взаимодействие с поставщиками и аналитиками данных
- Обеспечение сохранности данных (целостность, конфиденциальность)

**Навыки:**
- Управление архитектурой данных
- Разработка и управление хранилищами данных
- Понимание методов ИИ и машинного обучения
- Понимание существующих и потенциальных источников данных

#### Промпт-специалист или Инженер-оператор (Prompt Engineer)

**Область ответственности:**
- Бизнес-анализ
- Выяснение, как работает ИИ
- Использование продуманных подсказок с точными глаголами и словарным запасом для создания чат-ботов и других типов генеративного ИИ
- Доведение ИИ до предела возможностей, обнаруживая ошибки или новые проблемы
- Согласование поведения ИИ с человеческими намерениями

**Навыки:**
- Навыки бизнес-анализа
- Понимание методов ИИ и машинного обучения
- Владение синтаксисом естественного языка

### Варианты построения команд

#### Вариант 1. Крупная корпоративная система аналитики и прогностики

Продуктовый подход предпочтителен, требуется постоянная адаптация. Наиболее вероятна реализация на собственных мощностях.
...
*МСБ — малый и средний бизнес (Small and Medium Business, SMB)*
...
Использование готовых публичных ИИ-решений без разработки собственных моделей.
...
- Бизнес-аналитик
- Аналитик данных
- Специалист по моделям (датасаентист)/специалист по дискретной математике
- Инженер-разработчик
...
**Книги:**
- Эндрю Гласснер. Глубокое обучение без математики. Том I
- Machinelearning.ru
- Andrew Ng. Machine Learning yearning (Эндрю Ын «Страсть к машинному обучению» — https://habr.com/ru/post/419757/)
- Сергей Марков. Охота на электроовец

**Российские ресурсы по ИИ:**
- Российский портал ИИ — https://ai.gov.ru
- Российская ассоциация искусственного интеллекта (РАИИ) — http://www.raai.org/news/news.shtml
...
1. **Syllaby.io** — находите вирусные темы, пишите сценарии, создавайте видеоролики с использованием ИИ
2. **Claude.ai** — альтернатива ChatGPT
3. **zety.com** — конструктор резюме
4. **beautiful.ai** — создавайте красивые презентации
5. **hackertyper.net** — превратитесь в крутого хакера
6. **stockimg.ai** — дизайн и визуальный контент
7. **10web.io** — конструктор сайтов с ИИ
8. **fastreply.io** — увеличивайте количество подписчиков в Twitter/X
9. **cleanvoice.ai** — редактирование подкастов
10. **fastpedia.io** — каталог более 900 инструментов ИИ
