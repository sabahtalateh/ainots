...
<img src="../images/circs16.png">

Если разрезать на **бесконечно много бесконечно тонких** кусочков, можно "выпрямить" их в прямоугольник?

<img src="../images/circs128.png">

Кажется что можно, тогда высота такого прямоугольника будет равна $2r$ а ширина $\frac{C}{2}$ и мы сможем найти площадь.

Впервые сталкиваемся с **бесконечностью** в вычислениях.

**Проблема:** Это интуитивное рассуждение, не строгое доказательство! Что значит "бесконечно много"?
...
<details>
<summary><b>Как вычислялись площади многоугольников?</b> (раскрыть)</summary>

**Для вписанного правильного $n$-угольника:**

Архимед разбивал многоугольник на $n$ равнобедренных треугольников с вершиной в центре круга:

1. Каждый треугольник имеет:
   - Две стороны длиной $r$ (радиусы)
   - Угол при вершине $\frac{2\pi}{n}$

2. Площадь одного треугольника:
   $$S_{\triangle} = \frac{1}{2} r^2 \sin\left(\frac{2\pi}{n}\right)$$

3. Общая площадь вписанного $n$-угольника:
   $$S_{n} = n \cdot \frac{1}{2} r^2 \sin\left(\frac{2\pi}{n}\right) = \frac{nr^2}{2} \sin\left(\frac{2\pi}{n}\right)$$

**Для описанного правильного $n$-угольника:**

Каждая сторона касается круга. Треугольники имеют:
- Основание (сторона многоугольника) длиной $2r\tan\left(\frac{\pi}{n}\right)$
- Высоту $r$ (радиус круга)

Площадь описанного $n$-угольника:
$$S_n^{описанный} = nr\tan\left(\frac{\pi}{n}\right)$$

**Важно:** Архимед не использовал тригонометрию (она ещё не была разработана)! Он вычислял эти величины чисто геометрически, используя теорему Пифагора и подобие треугольников.
...
### Как Архимед вычислял площадь многоугольника?

Для правильного $n$-угольника, вписанного в круг радиуса $r$:

1. Разбиваем на $n$ равнобедренных треугольников
2. Площадь одного треугольника: $S_{\triangle} = \frac{1}{2} r^2 \sin\left(\frac{2\pi}{n}\right)$
3. Общая площадь: $S_n = n \cdot \frac{1}{2} r^2 \sin\left(\frac{2\pi}{n}\right)$

При $n \to \infty$:

$$S_n = \frac{n}{2} r^2 \sin\left(\frac{2\pi}{n}\right) \to \pi r^2$$

**Замечание:** Архимед не знал тригонометрии и синусов! Он использовал геометрические рассуждения.
...
## Метод исчерпывания vs Интеграл

### Сравнение подходов

| Характеристика | Метод Архимеда | Современный интеграл |
|---------------|----------------|---------------------|
| **Эпоха** | III век до н.э. | XVII-XIX век н.э. |
| **Идея** | Вписанные/описанные многоугольники | Суммы Римана |
| **Запись** | Геометрическая (рисунки) | Алгебраическая: $\int_a^b f(x)\,dx$ |
| **Предел** | Неявный (доказательство от противного) | Явный: $\lim_{n \to \infty}$ |
| **Общность** | Каждая задача индивидуально | Общий метод для любой функции |
| **Результаты** | $S_{круга} = \pi r^2$, $S_{параболы} = \frac{1}{3}$ | Любые площади и объёмы |

### Современная интерпретация

Метод исчерпывания — это **прото-интеграл**!

Архимед фактически вычислял:

$$S_{круга} = \lim_{n \to \infty} S_{n\text{-угольника}}$$

Но не имел **языка**, чтобы это записать!

---

## Примеры задач в духе Архимеда

### Задача 1: Площадь круга через вписанные многоугольники

Вычислите площадь круга радиуса $r = 1$, используя:

**а)** Вписанный правильный 6-угольник  
**б)** Вписанный правильный 12-угольник  
**в)** Сравните с $\pi \approx 3.14159$

**Подсказка:** Правильный $n$-угольник состоит из $n$ равнобедренных треугольников с углом $\frac{2\pi}{n}$ при вершине.

### Задача 2: Площадь сегмента параболы

Найдите площадь под параболой $y = x^2$ на отрезке $[0, 1]$ методом прямоугольников:

**а)** Разбейте отрезок на 4 части, используйте левые концы  
**б)** Разбейте отрезок на 4 части, используйте правые концы  
**в)** Что происходит при увеличении числа разбиений?

**Ожидаемый ответ:** $\frac{1}{3}$ (как у Архимеда!)

### Задача 3: Длина окружности

Используя периметры вписанных правильных многоугольников, оцените длину окружности радиуса $r = 1$.

**Подсказка:** Периметр правильного $n$-угольника: $P_n = 2nr\sin\left(\frac{\pi}{n}\right)$

---

## Визуализация метода

### ASCII-диаграмма: от 6-угольника к кругу

```
Шаг 1: n=6           Шаг 2: n=12          Шаг 3: n=24
    /\                   /\                   /‾\
   /  \                /    \               /    \
  |    |              /      \             /      \
  |  • |             |    •   |           |    •   |
...
При n → ∞ многоугольник → окружность
```
