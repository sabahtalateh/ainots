...
Асимптотические последовательности очень похоже ведут себя при большом значении $n$. Две последовательности $A$ и $B$ называются **асимптотически равными** (или асимптотически эквивалентными), если:

$$\lim_{n \rightarrow \infty} \frac{A_n}{B_n} = 1$$

В этом случае можно использовать тильду:

$$A \sim B$$

Можно заметить, что если $A$ и $B$ асимптотически эквивалентны, то их пределы $a$ и $b$ будут равны, так как:
$$
...
$$
так как мы знаем, что $\frac{1}{n}$ стремится к нулю. Поэтому $A_n$ и $B_n$ — асимптотически эквивалентны.
