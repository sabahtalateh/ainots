...
## Определение в ε-N (для бесконечности). Развёрнуто и с пояснениями
...
Число $A$ есть предел последовательности $x_n$ при $n$, стремящемся к бесконечности ($n \rightarrow \infty$), если для любого $\varepsilon$ (маленького и положительное), что начиная с некоторого номера $N_\varepsilon$ (номера элементов этой последовательности), все элементы последовательности (следующие за этим номером) ($\forall x_n$ при $n > N_\varepsilon$), будут находиться на расстоянии меньше указанного маленького числа $\varepsilon$ от значения предела ($|x_n - A| < \varepsilon$).

**Это определение, тут ничего доказывать не нужно**

### Вычисление и доказательство

Рассмотрим как пользоваться этими сакральными информациями. Имея данное определение предел нужно:
1. Вычислить, используя алгебраические преобразования
2. Доказать что предел функции равен вычисленному значению
...
Теперь докажем строго по определению, что предел действительно равен $2$.
...
- **δ (дельта)** — расстояние по оси **X** (горизонталь)
...
| **Примеры** | $\lim_{n \to \infty} \frac{1}{n} = 0$ <br> $\lim_{x \to \infty} \frac{1}{x} = 0$ | $\lim_{x \to 2} \frac{1}{x} = \frac{1}{2}$ |
