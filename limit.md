...
Число $A$ есть предел последовательности $x_n$ при $n$, стремящемся к бесконечности ($n \rightarrow \infty$), если для любого $\varepsilon$ (на практике маленького и положительного числа), что начиная с некоторого номера $N_\varepsilon$ (номера элементов этой последовательности), все элементы последовательности (следующие за этим номером) ($\forall x_n$ при $n > N_\varepsilon$), будут находиться на расстоянии меньше указанного маленького числа $\varepsilon$ от значения предела ($|x_n - A| < \varepsilon$).
