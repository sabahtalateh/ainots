...
- День 1
  - [Множества](./specmatan/matan1/day1/Sets.ipynb)
  - [Множества д.з.](./specmatan/matan1/day1/Sets_tasks.ipynb)
- День 2
  - [Symbolic Python](./specmatan/matan1/day2/SymPy_Basic.ipynb)
  - [Пределы](./specmatan/matan1/day2/Matan1_day2.ipynb)
...
Ревизии — это слепки состояния окружения (установленных пакетов).
...
**Внимание:** это меню долго обновляется, можно указать путь до питона руками (можно перезапустить IDE).

**Шаг 4: Интерпретатор для VSCode/Cursor**

Для корректной работы **IntelliSense** при импорте библиотек, установленных в выбранном для ноутбука окружении, необходимо установить для всего редактора тот же интерпретатор, что установлен для ноутбука.

1. В палитре команд `Cmd+Shift+P`
2. Пишем `Python: Select Interpreter`
3. Выбираем интерпретатор, установленный для ноутбука.
...
**Шаг 6: Проверка доступных пакетов**
...
## SymPy. Символьный Python

Библиотека для работы с символьной математикой, для вычислений строятся графы, которые вычисляются с минимальной потерей точности, за счёт того, что вычисления до последнего шага производятся в простых дробях.

### Зачем нужна SymPy?

SymPy позволяет работать с математикой **аналитически** (символьно), а не **численно**. Это значит, что вы получаете точные математические выражения, а не приближённые числа.

**Примеры использования:**

#### 1. Символьные вычисления и упрощение выражений
...
simplified = sp.simplify(expr2)  # 1
```
...
# Производная
f = x**3 + 2*x**2 - 5*x + 1
derivative = sp.diff(f, x)  # 3*x**2 + 4*x - 5

# Интеграл
integral = sp.integrate(x**2, x)  # x**3/3

# Определённый интеграл
definite = sp.integrate(x**2, (x, 0, 1))  # 1/3 (точное значение!)
```
...
solutions = sp.solve(equation, x)  # [2, 3]

# Система уравнений
x, y = sp.symbols('x y')
eq1 = sp.Eq(x + y, 5)
eq2 = sp.Eq(x - y, 1)
solution = sp.solve([eq1, eq2], [x, y])  # {x: 3, y: 2}
```
...
# Предел функции
limit = sp.limit(sp.sin(x)/x, x, 0)  # 1

# Предел на бесконечности
limit_inf = sp.limit((1 + 1/x)**x, x, sp.oo)  # e
```

#### 5. Высокоточные вычисления

```python
# π с 100 знаками после запятой
pi_100 = sp.N(sp.pi, 100)

# √2 с точностью 50 знаков
sqrt2 = sp.N(sp.sqrt(2), 50)
```
...
# NumPy - численные вычисления (приближённые)
np.sqrt(2)  # 1.4142135623730951

# SymPy - символьные вычисления (точные)
sp.sqrt(2)  # sqrt(2) - сохраняет как символ
sp.N(sp.sqrt(2), 50)  # можно получить любую точность
```

**Итог:** SymPy используется, когда нужны точные математические выражения, а не приближённые числа. Идеально для обучения, исследований и проверки аналитических решений.
