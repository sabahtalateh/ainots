...
**Неформальное определение:** Говорят что число ($A$) является пределом последовательности ($x_n$) при $n$, стремящемся к бесконечности ($n \rightarrow \infin$), если можно задать такое число ($\exists \varepsilon$) (маленькое и положительное) , что начиная с некоторого номера ($N_\varepsilon$), все элементы последовательности, следующие за этим номером ($\forall x_n$ при $n > N_\varepsilon$) будут находиться на расстоянии меньше указанного маленького числа ($\varepsilon$) от значения предела ($│x_n - A│< \varepsilon$). 

**Иллюстрации:**

| | | |
|:---:|:---:|:---:|
| ![](./images/lime02.png) | ![](./images/lime01.png) | ![](./images/lime005.png) |
| $\varepsilon = 0.2$ | $\varepsilon = 0.1$ | $\varepsilon = 0.05$ |
