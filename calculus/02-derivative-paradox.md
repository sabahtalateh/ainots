...
Найдём скорость в момент $t = 2$ для функции $s(t) = t^3$.

**Вычисляем отношение:**

$$\frac{s(2 + dt) - s(2)}{dt} = \frac{(2 + dt)^3 - 8}{dt}$$

**Раскрываем $(2 + dt)^3$:**

$$(2 + dt)^3 = 8 + 12 \cdot dt + 6 \cdot dt^2 + dt^3$$

**Вычитаем 8 и делим на $dt$:**

$$\frac{12 \cdot dt + 6 \cdot dt^2 + dt^3}{dt} = 12 + 6 \cdot dt + dt^2$$

**Берём предел при $dt \to 0$:**

При $dt \to 0$ члены $6 \cdot dt$ и $dt^2$ исчезают, остаётся только $12$.
