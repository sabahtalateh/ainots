...
Пока что это "загадочная функция" — мы знаем, что она существует (площадь же есть!), но не знаем её формулу. Инструменты для её нахождения — главная тема матанализа.
...
$$\boxed{\frac{dA}{dx} \approx x^2}$$

Эта запись обозначает **изменение площади при изменении $x$** и оно равно просто нашей функции (в любой точке $x$) или **высоте** тонкого **прямоугольника под графиком** в некоторой точке.
...
Отношение $\frac{dA}{dx} = \frac{A(3.001) - A(3)}{0.001}$ должно быть примерно равно $3^2 = 9$. Пока что мы не знаем что за функция $A$ так что мы только выяснили её полезное свойство.
...
- **Интеграл** — это площадь под графиком функции $f(x)$
- **Производная** функции площади $A(x)$ равна самой функции $f(x)$
...
Мы знаем, что производная $\frac{x^3}{3}$ равна $x^2$:

$$\left(\frac{x^3}{3}\right)' = x^2$$

Значит, площадь под графиком $y = x^2$ от $0$ до $x$ равна:

$$A(x) = \frac{x^3}{3}$$

Мы нашли "загадочную функцию" из начала! Без неё пришлось бы суммировать бесконечно много прямоугольников вручную.
