...
- **Интеграл** — $A(x)$ — это площадь под графиком функции $f(x)$ (пока это всё ещё "загадочная" функция)
- **Производная** функции этой площади $\frac{A(x)}{dx}$ равна самой функции $f(x)$

$$\boxed{\frac{A(x)}{dx} = f(x)}$$
