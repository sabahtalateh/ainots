# Полный вывод формулы Симпсона через полином Лагранжа

## 1. Постановка задачи

Требуется приблизить интеграл

$$
\int_a^b f(x)dx
$$

по трём узлам: $x_0 = a$, $x_1 = \frac{a+b}{2}$, $x_2 = b$. Введём шаг сетки:

$$
h = \frac{b-a}{2},
$$

так что $x_1 - x_0 = x_2 - x_1 = h$.

---

## 2. Интерполяционный полином Лагранжа

Строим полином второй степени $P_2(x)$, совпадающий с $f$ в узлах $x_0,x_1, x_2$:

$$
P_2(x) = f(x_0)L_0(x) + f(x_1)L_1(x) + f(x_2)L_2(x),
$$

где $L_i(x)$ — базисные полиномы Лагранжа:

$$
L_i(x) = \prod_{j \neq i} \frac{x - x_j}{x_i - x_j}, \quad i = 0,1,2.
$$

В явном виде:

- $L_0(x) = \dfrac{(x-x_1)(x-x_2)}{(x_0-x_1)(x_0-x_2)}$;
- $L_1(x) = \dfrac{(x-x_0)(x-x_2)}{(x_1-x_0)(x_1-x_2)}$;
- $L_2(x) = \dfrac{(x-x_0)(x-x_1)}{(x_2-x_0)(x_2-x_1)}$.

Приближение интеграла:

$$
\int_a^b f(x)dx \approx \int_a^b P_2(x)dx = f(x_0)\int_a^b L_0(x)dx + f(x_1)\int_a^b L_1(x)dx + f(x_2)\int_a^b L_2(x)dx.
$$

Обозначим веса

$$
w_i = \int_a^b L_i(x)dx.
$$

Тогда

$$
\int_a^b f(x)dx \approx w_0 f(x_0) + w_1 f(x_1) + w_2 f(x_2).
$$

Нужно вычислить $w_0, w_1, w_2$.

---

## 3. Замена переменной $u = x - x_1$

**Что мы интегрируем.** Нам нужно посчитать веса

$$
w_i = \int_a^b L_i(x)dx
$$

— это интегралы от базисных полиномов $L_i(x)$ по переменной $x$ от $a$ до $b$. После замены переменной мы будем интегрировать ту же величину, но записанную через $u$ и $du$, с новыми пределами.

**Вводим новую переменную:**

$$
u = x - x_1 = x - \frac{a+b}{2}.
$$

То есть $u$ — это смещение точки $x$ относительно середины отрезка $[a,b]$. Переменная $u$ зависит от $x$; $x_1$ — константа.

**Почему $\frac{du}{dx} = 1$.** Берём производную $u$ по $x$:

$$
\frac{du}{dx} = \frac{d}{dx}\bigl(x - x_1\bigr) = 1,
$$

потому что производная по $x$ от $x$ равна 1, а от константы $x_1$ — нулю.

**Почему $du = dx$.** Из равенства $\frac{du}{dx} = 1$ записываем дифференциалы: $du = 1\cdot dx = dx$. Поэтому при замене переменной в интеграле мы подставляем $du$ вместо $dx$ без дополнительного множителя.

**Как меняются пределы.** Когда $x$ пробегает отрезок от $a$ до $b$, переменная $u = x - x_1$ пробегает отрезок от $u(a) = a - x_1$ до $u(b) = b - x_1$. Поскольку $x_1 = \frac{a+b}{2}$ и $h = \frac{b-a}{2}$, получаем $u(a) = -\frac{b-a}{2} = -h$ и $u(b) = \frac{b-a}{2} = h$. Итого:

$$
\int_a^b L_i(x)dx = \int_{-h}^{h} L_i(u)du.
$$

Слева мы интегрируем по $x$ от $a$ до $b$, справа — по $u$ от $-h$ до $h$; под интегралом стоит одна и та же функция, записанная через старую или новую переменную, и $dx$ заменён на $du$.

**Связь старых и новых переменных:**
- $x - x_0 = x - (x_1 - h) = u + h$;
- $x - x_1 = u$;
- $x - x_2 = x - (x_1 + h) = u - h$.

Знаменатели в $L_i$ (разности узлов) от $x$ не зависят: $x_0 - x_1 = -h$, $x_0 - x_2 = -2h$, $x_1 - x_0 = h$, $x_1 - x_2 = -h$, $x_2 - x_0 = 2h$, $x_2 - x_1 = h$. Подставляя в формулы для $L_i$, получаем выражения как функции от $u$:

$$
L_0(u) = \frac{u(u-h)}{(-h)(-2h)} = \frac{u(u-h)}{2h^2},
$$

$$
L_1(u) = \frac{(u+h)(u-h)}{h\cdot(-h)} = \frac{u^2 - h^2}{-h^2} = \frac{h^2 - u^2}{h^2},
$$

$$
L_2(u) = \frac{(u+h)u}{(2h)h} = \frac{u(u+h)}{2h^2}.
$$

---

## 4. Вычисление интегралов $\displaystyle \int_{-h}^{h} L_i(u)du$

**Интеграл для $L_0$:**

$$
\int_{-h}^{h} \frac{u(u-h)}{2h^2}du = \frac{1}{2h^2} \int_{-h}^{h} (u^2 - uh)du.
$$

Функция $u^2$ чётная, $uh$ нечётная; интеграл по $[-h,h]$ от нечётной равен нулю. Поэтому

$$
\int_{-h}^{h} (u^2 - uh)du = \int_{-h}^{h} u^2du = 2\int_0^h u^2du = 2\cdot\frac{h^3}{3} = \frac{2h^3}{3},
$$

и

$$
w_0 = \frac{1}{2h^2}\cdot\frac{2h^3}{3} = \frac{h}{3}.
$$

**Интеграл для $L_1$:**

$$
\int_{-h}^{h} \frac{h^2 - u^2}{h^2}du = \frac{1}{h^2} \int_{-h}^{h} (h^2 - u^2)du.
$$

Подынтегральная функция чётная:

$$
\int_{-h}^{h} (h^2 - u^2)du = 2\int_0^h (h^2 - u^2)du = 2\left[ h^2 u - \frac{u^3}{3} \right]_0^h = 2\left( h^3 - \frac{h^3}{3} \right) = \frac{4h^3}{3},
$$

и

$$
w_1 = \frac{1}{h^2}\cdot\frac{4h^3}{3} = \frac{4h}{3}.
$$

**Интеграл для $L_2$:**

$$
\int_{-h}^{h} \frac{u(u+h)}{2h^2}du = \frac{1}{2h^2} \int_{-h}^{h} (u^2 + uh)du.
$$

Снова $u^2$ даёт вклад, $uh$ — нет:

$$
\int_{-h}^{h} (u^2 + uh)du = \int_{-h}^{h} u^2du = \frac{2h^3}{3},
$$

и

$$
w_2 = \frac{1}{2h^2}\cdot\frac{2h^3}{3} = \frac{h}{3}.
$$

Итого: $w_0 = w_2 = \dfrac{h}{3}$, $w_1 = \dfrac{4h}{3}$.

---

## 5. Формула Симпсона

Подставляем веса и $x_0 = a$, $x_1 = \frac{a+b}{2}$, $x_2 = b$, $h = \frac{b-a}{2}$:

$$
\int_a^b f(x)dx \approx \frac{h}{3}f(a) + \frac{4h}{3}f\!\left(\frac{a+b}{2}\right) + \frac{h}{3}f(b).
$$

Выносим $\frac{h}{3}$ и заменяем $h = \frac{b-a}{2}$:

$$
\boxed{
\int_a^b f(x)dx \approx \frac{b-a}{6}\left( f(a) + 4f\!\left(\frac{a+b}{2}\right) + f(b) \right).
}
$$

Это и есть **формула Симпсона** (квадратурная формула по трём узлам с весами 1 : 4 : 1).

---

## Дополнение: замена $dx$ на $du$ (подробнее)

### Связь между $x$ и $u$

Мы вводим новую переменную:

$$
u = x - x_1 = x - \frac{a+b}{2}.
$$

$x_1$ — константа, поэтому $u$ зависит только от $x$. Берём производную по $x$:

$$
\frac{du}{dx} = \frac{d}{dx}(x - x_1) = 1.
$$

Умножаем на $dx$ (в смысле записи дифференциалов):

$$
du = 1\cdot dx = dx.
$$

То есть **дифференциалы совпадают: $du = dx$**. При замене переменной в интеграле мы просто пишем $du$ вместо $dx$.

### Как это используется в интеграле

Исходный интеграл по $x$:

$$
\int_a^b L_i(x)dx.
$$

Подстановка:
- выражение под интегралом переписываем как функцию от $u$: $L_i(x) = L_i(u)$ (через $x - x_0 = u+h$, $x-x_1 = u$, $x-x_2 = u-h$);
- по правилу замены переменной пишем $dx = du$, то есть

$$
\int_a^b L_i(x)dx = \int_{u(a)}^{u(b)} L_i(u)du;
$$

- при $x = a$ получается $u = -h$, при $x = b$ — $u = h$, поэтому

$$
\int_a^b L_i(x)dx = \int_{-h}^{h} L_i(u)du.
$$

Итого: **«замена $dx$ на $du$»** — это то, что $du = dx$ (коэффициент 1), а пределы мы пересчитываем по формуле $u = x - x_1$.

### Почему именно так

По определению, при замене $x = \varphi(u)$ пишут:

$$
\int f(x)dx = \int f(\varphi(u))\varphi'(u)du.
$$

У нас $x = u + x_1$, то есть $\varphi(u) = u + x_1$, поэтому $\varphi'(u) = 1$ и

$$
dx = \varphi'(u)du = 1\cdot du = du.
$$

Так что замена **$dx$ на $du$** — это не договорённость, а следствие того, что новая переменная введена по формуле $u = x - \text{const}$: производная по $x$ равна 1, значит $du = dx$.
